...
# --- KONFIGURASI BOT ---
# Ambil token dari Environment Variable Render, atau gunakan fallback jika tidak ada
...
@app_flask.route('/')
def home():
    return "Bot Telegram Keuangan Matthew Aktif 24/7!"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "BCA" in text and "BLU" not in text:
        ...
        await update.message.reply_text("📊 *Rekapitulasi Total:* Fitur integrasi Google Sheets aktif.", parse_mode="Markdown")
...
if __name__ == "__main__":
    ...
    # 2. Inisialisasi Event Loop Eksplisit (Mengatasi RuntimeError Python 3.14 / Asyncio)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)

    # 3. Jalankan Telegram Bot Polling
